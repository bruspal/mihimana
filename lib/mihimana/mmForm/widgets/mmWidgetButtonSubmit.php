...
class mmWidgetButtonSubmit extends mmWidgetButton {
  ...
  public function __construct($libelle = 'Valider', $preSubmit = '', $name = '', $attributs = array()) {
    if ( ! $name) {
  ...
  public function render($extraAttributes = array(), $replace = false) {
    ...
    if (AJAX_REQUEST)
    {
      $parameters = array('onclick'=>sprintf("%smdAjxSubmit($('form#%s'))", $this->preSubmit, $this->containerForm->getId()));
    }
    else
    {
