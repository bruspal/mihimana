...
//gestion des pages          
  ...
  public function render($extraAttributes = array(), $replace = false)
  {
    ...
    if ($this->rendered) return '';  //On authorise qu'un seule et unique rendue du widget par page. Si il est marqué comme deja rendue, retour une chaine vide
...
//<---- Sortie ici. Si on a d'enregistrement on renvoie une chaine vide. cad le widget n'est pas generé      
    ...
    if ($this->firstCall)
    {
      ...
      $result = sprintf('<div id="%s"><table class="%s" style="width: %s">%s%s%s</table>%s</div>',
              $this->getId(), $this->getAttribute('class'), $largeurTotale, $header, $body, $footer, $htmlNavigation);
    }
    else
    {
      //sinon on met a jour que le contenu
      $result = sprintf('<table class="%s" style="width: %s">%s%s%s</table>%s',
              $this->getAttribute('class'), $largeurTotale, $header, $body, $footer, $htmlNavigation);
    }
