...
class pWs extends mmProgWebService {
  ...
  public function popupSelectTable(mmRequest $postget)
  {
    
  }
  
  public function cherchePremierEnreg(mmRequest $postget)
  {
    $table = $postget->getParam('t', false);
    if ($table == false)
    {
      return json_encode(array('success'=>false, 'message'=>'Table manquante'));
    }
    $cle = $postget->getParam('c', false);
    if ($cle == false)
    {
      return json_encode(array('success'=>false, 'message'=>'cle manquante'));
    }
    $valeur = $postget->getParam('v', false);
    if ($valeur === false)
    {
      return json_encode(array('success'=>false, 'message'=>'valeur manquante'));
    }
    if($postget->getParam('s', false) === false)
    {
      $select = '*';
    }
    else
